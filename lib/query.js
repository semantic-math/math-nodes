...
export const isFraction = (node) => query.isNeg(node) ? query.isFraction(node.args[0]) : query.isDiv(node)
...
export const getNumerator = (node) => query.isNeg(node) ? getNumerator(node.args[0]) : node.args[0]
export const getDenominator = (node) => query.isNeg(node) ? getDenominator(node.args[0]) : node.args[1]
